<template>
  <v-container class="pa-4" fluid>
    <v-card :loading="loading">
      ...
      <v-card-text>
        <v-textarea v-model="domains" filled ref="domains" label="Domains"
          :placeholder="'example.com\ngoogle.com\nmagic.com'" hint="Enter one domain name per line." :rules="rules"
          persistent-hint />
      </v-card-text>
      <v-card-actions>
    ...
    <v-alert v-if="created" dense text type="success">
...
export default {
  ...
  methods: {
     ...
     */
     async parseAndSaveDomains(domains) {
       ...
       for (let i = 0; i < domainList.length; i++) {
         ...
         try {
           ...
           await this.$axios.post(`/domains/`, {
            //  "id": Math.floor(Math.random() * 1000000),
             "id": uuid.v4(),
             "domain_name": domain,
             "user_id": uuid.v4(),
             "domain_name_no_tld": "unsoldcarsforsale",
             "time_added_to_user": timeStamp,
             "is_idn": false,
             "imprint": true,
             "folder_id": null,
             "folder_name": "Unassigned"
